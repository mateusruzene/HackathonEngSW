          ...
          {dashboard?.estatisticas && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
                <span className="text-xs font-medium text-slate-500 block mb-0.5">Equipes</span>
                <span className="text-lg font-bold text-slate-900">
                  {dashboard.estatisticas.totalEquipes}/{dashboard.estatisticas.maxEquipes}
                </span>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
                <span className="text-xs font-medium text-slate-500 block mb-0.5">Projetos</span>
                <span className="text-lg font-bold text-blue-700">
                  {dashboard.estatisticas.totalProjetos}
                </span>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
                <span className="text-xs font-medium text-slate-500 block mb-0.5">Mentorias</span>
                <span className="text-lg font-bold text-slate-700">
                  {dashboard.estatisticas.totalMentorias}
                </span>
              </div>
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
                <span className="text-xs font-medium text-slate-500 block mb-0.5">Avaliações</span>
            ...
            {top1 ? (
              <div className="bg-amber-50/50 border-2 border-amber-400 rounded-xl p-5 shadow-sm flex flex-col justify-between space-y-3">
                ...
                <div>
                  ...
                  <span className="inline-block mt-2 text-xs bg-white px-2 py-0.5 rounded border border-amber-200 text-amber-800 font-medium">
                    {top1.areaTematica}
                  </span>
                </div>
            ...
            {top2 ? (
              <div className="bg-white border border-slate-300 rounded-xl p-5 shadow-sm flex flex-col justify-between space-y-3">
                ...
                <div>
                  ...
                  <span className="inline-block mt-2 text-xs bg-slate-100 px-2 py-0.5 rounded text-slate-700">
                    {top2.areaTematica}
                  </span>
                </div>
        ...
        {ranking.length > 0 ? (
              ...
              <div key={item.equipeId} className="p-5 hover:bg-slate-50/60 transition-colors space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${item.posicao === 1 ? 'bg-amber-400 text-amber-950' :
                ...
                <div className="bg-slate-50 rounded-lg p-3.5 border border-slate-200 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs uppercase font-semibold bg-white text-slate-600 px-2 py-0.5 rounded border border-slate-200">
                      {item.areaTematica}
                    </span>
                  </div>
                        ...
                        {item.avaliacoes.map((av, i) => (
                          <div key={i} className="text-xs bg-white p-2.5 rounded-lg border border-slate-200">
                            <div className="flex items-center justify-between font-semibold text-slate-800 mb-0.5">
                              ...
                              <span className="text-blue-700 font-bold text-xs">Nota {Number(av.nota).toFixed(1)}</span>
                            </div>
                            <p className="text-slate-600 text-xs italic">"{av.comentarios || 'Sem comentários adicionais.'}"</p>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div className="p-10 text-center space-y-3">
            ...
            <button
              ...
              className="inline-flex items-center px-3.5 py-2 rounded-lg bg-blue-700 hover:bg-blue-800 text-white font-medium text-xs transition-colors cursor-pointer"
            >
