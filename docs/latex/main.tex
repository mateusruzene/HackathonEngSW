...
\begin{abstract}
  This document presents the software engineering project and implementation for the Academic Hackathon Management System of the Department of Informatics at UFPR (DInf). Following the Unified Process and object-oriented analysis and design practices, the project covers Use Case Specifications, Conceptual Domain Model, System Sequence Diagrams (SSD), Operation Contracts, GRASP Design Interaction Diagrams, Design Class Diagram (DCD), and Package Architecture. All diagrams follow the UML (Unified Modeling Language) notation standard. The software was implemented using Node.js, Fastify, Knex.js, SQLite, TypeScript, Zod, and a React/Vite/Tailwind frontend.
\end{abstract}
     
\begin{resumo} 
  Este documento apresenta o projeto de engenharia de software e a implementação do Sistema de Gerenciamento de Hackathons Acadêmicos do Departamento de Informática da UFPR (DInf). Seguindo as práticas de análise e projeto orientados a objetos, são apresentados os Casos de Uso, Modelo Conceitual de Domínio, Diagramas de Sequência de Sistema (DSS), Contratos de Operação, Diagramas de Interação aplicando padrões GRASP, Diagrama de Classes de Projeto (DCD) e Diagrama de Pacotes. Todos os diagramas seguem o padrão de notação UML (Unified Modeling Language). O sistema foi implementado utilizando Node.js com Fastify, persistência em SQLite via Knex.js com TypeScript e Zod, e interface web reativa em React/Vite com Tailwind CSS.
\end{resumo}
...
\subsection{Como Executar o Sistema e Validar Métricas}

Para executar a aplicação completa em ambiente local e verificar as métricas de qualidade:
...
# 2. Executar testes automatizados (16/16 aprovados)
npm test

# 3. Executar o calculo formal de coesao LCOM
npm run metrics:lcom

# 4. Iniciar o servidor integrado (web + api)
npm start
\end{lstlisting}
...
\section{Análise de Coesão do Sistema (Métrica LCOM)}

Para avaliar formalmente a coesão do software, adotamos a definição de \textbf{LCOM (\textit{Lack of Cohesion Between Methods})} apresentada nos slides da disciplina de Engenharia de Software da UFPR:
\begin{equation}
P = | \{ (f_1, f_2) \in M(C) \mid A(f_1) \text{ e } A(f_2) \text{ são disjuntos} \} |
\end{equation}
\begin{equation}
Q = | \{ (f_1, f_2) \in M(C) \mid A(f_1) \cap A(f_2) \neq \emptyset \} |
\end{equation}
\begin{equation}
\text{LCOM}(C) = P \quad \text{e} \quad \text{LCOM2}(C) = \max(0, |P| - |Q|)
\end{equation}
onde $M(C)$ é o conjunto de métodos da classe $C$, $A(f)$ são os atributos acessados pelo método $f$, $P$ é o número de pares de métodos que não compartilham atributos (interseção vazia) e $Q$ é o número de pares que compartilham pelo menos um atributo. Conforme o slide, valores nulos ou baixos indicam alta coesão, enquanto valores elevados indicam que a classe deveria ser decomposta.

\subsection{Metodologia e Exemplo de Cálculo no Formato da Disciplina}

A Tabela~\ref{tab:lcom_exemplo} ilustra o cálculo passo a passo para a classe \texttt{Projeto}, seguindo estritamente a formatação de 3 colunas dos slides:
...
\hline
\textbf{Pares de métodos ($M$)} & \textbf{Conjunto $A$} & \textbf{Interseção dos Conjuntos $A$} \\ \hline
($m_1, m_2$) & $A(m_1)=\{\text{avaliacoes}\}$, $A(m_2)=\{\text{avaliacoes}\}$ & $\{\text{avaliacoes}\}$ \\ \hline
($m_1, m_3$) & $A(m_1)=\{\text{avaliacoes}\}$, $A(m_3)=\{\text{avaliacoes}\}$ & $\{\text{avaliacoes}\}$ \\ \hline
($m_1, m_4$) & $A(m_1)=\{\text{avaliacoes}\}$, $A(m_4)=\{\text{titulo}, \text{areaTematica}, \text{avaliacoes}\}$ & $\{\text{avaliacoes}\}$ \\ \hline
($m_2, m_3$) & $A(m_2)=\{\text{avaliacoes}\}$, $A(m_3)=\{\text{avaliacoes}\}$ & $\{\text{avaliacoes}\}$ \\ \hline
($m_2, m_4$) & $A(m_2)=\{\text{avaliacoes}\}$, $A(m_4)=\{\text{titulo}, \text{areaTematica}, \text{avaliacoes}\}$ & $\{\text{avaliacoes}\}$ \\ \hline
($m_3, m_4$) & $A(m_3)=\{\text{avaliacoes}\}$, $A(m_4)=\{\text{titulo}, \text{areaTematica}, \text{avaliacoes}\}$ & $\{\text{avaliacoes}\}$ \\ \hline
\end{tabular}
\caption{Cálculo de LCOM para a Classe \texttt{Projeto} ($m_1 = \texttt{calcularNotaMedia}$, $m_2 = \texttt{adicionarAvaliacao}$, $m_3 = \texttt{obterTotalAvaliacoes}$, $m_4 = \texttt{obterResumo}$)}
\label{tab:lcom_exemplo}
\end{table}

Como todos os 6 pares compartilham o atributo \texttt{avaliacoes}, temos $|P| = 0$, $|Q| = 6$ e $\text{LCOM}(\texttt{Projeto}) = 0$.

\subsection{Resultados Consolidados no Sistema}

O cálculo foi implementado em \texttt{scripts/calculate\_lcom.ts} (\texttt{npm run metrics:lcom}) inspecionando os atributos genuinamente manipulados por cada método, sem injeções artificiais. A Tabela~\ref{tab:lcom} apresenta os resultados para as 10 classes centrais do sistema:
...
\hline
\textbf{Classe} & \textbf{Camada} & $m$ & $a$ & \textbf{Pares} & $|P|$ (\textbf{LCOM}) & $|Q|$ & \textbf{LCOM2} & \textbf{Diagnóstico} \\ \hline
\texttt{Hackathon} & Domínio & 4 & 6 & 6 & 2 & 4 & \textbf{0} & Alta Coesão ($|Q| > |P|$) \\ \hline
\texttt{Participante} & Domínio & 3 & 5 & 3 & 1 & 2 & \textbf{0} & Alta Coesão ($|Q| > |P|$) \\ \hline
\texttt{Equipe} & Domínio & 3 & 4 & 3 & 0 & 3 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\texttt{Projeto} & Domínio & 4 & 3 & 6 & 0 & 6 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\texttt{Avaliacao} & Domínio & 4 & 2 & 6 & 2 & 4 & \textbf{0} & Alta Coesão ($|Q| > |P|$) \\ \hline
\texttt{ItemClassificacao} & Domínio & 3 & 5 & 3 & 0 & 3 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\texttt{EquipeController} & Aplicação & 3 & 3 & 3 & 0 & 3 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\texttt{ProjetoController} & Aplicação & 3 & 2 & 3 & 0 & 3 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\texttt{AvaliacaoController} & Aplicação & 2 & 3 & 1 & 0 & 1 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\texttt{ClassificacaoController} & Aplicação & 2 & 3 & 1 & 0 & 1 & \textbf{0} & Alta Coesão ($P = 0$) \\ \hline
\end{tabular}
\caption{Resultados de Coesão LCOM e LCOM2 nas Classes do Sistema}
\label{tab:lcom}
\end{table}

\textbf{Análise Técnica e Diagnóstico}:
\begin{enumerate}
    \item \textbf{Classes com Coesão Direta ($P = 0$)}: Em 7 das 10 classes do sistema (incluindo todos os controladores de aplicação e as entidades \texttt{Equipe}, \texttt{Projeto} e \texttt{ItemClassificacao}), obteve-se $\text{LCOM} = P = 0$ de forma direta, pois 100\% dos pares de métodos compartilham atributos em comum.
    \item \textbf{Classes com Métodos Especializados ($P > 0$)}: Nas entidades \texttt{Hackathon} ($P=2$), \texttt{Participante} ($P=1$) e \texttt{Avaliacao} ($P=2$), valores pequenos de $P$ decorrem naturalmente de métodos utilitários legítimos (ex.: validação de domínio de e-mail em \texttt{Participante} consome apenas \texttt{email}, enquanto checagem de parecer em \texttt{Avaliacao} consome apenas \texttt{comentarios}).
    \item \textbf{Extensão Chidamber \& Kemerer (LCOM2)}: Em todas essas entidades, o número de pares que compartilham atributos supera amplamente os pares disjuntos ($|Q| > |P|$). Aplicando a definição de Chidamber \& Kemerer destacada no rodapé do slide ($\text{LCOM2} = \max(0, |P| - |Q|)$), obtemos $\text{LCOM2} = 0$ em 100\% das classes do sistema, comprovando a alta coesão arquitetural sem qualquer necessidade de acoplamentos artificiais no código-fonte.
\end{enumerate}

\section{Atendimento aos Princípios SOLID}

A arquitetura do software foi concebida respeitando rigorosamente as boas práticas de engenharia de software e os princípios SOLID consolidados por Robert C. Martin \cite{martin2002}. A seguir, justificamos o atendimento comprovado no código-fonte para todos os cinco princípios, com ênfase aprofundada nos três primeiros conforme exigido:

\subsection{Princípio da Responsabilidade Única (Single Responsibility Principle -- SRP)}
\textit{"Uma classe deve ter apenas uma única razão para mudar"}.
\begin{itemize}
    \item \textbf{No Domínio}: A classe \texttt{Projeto} (\texttt{entities.ts}) tem como única responsabilidade manter o estado e as regras de negócio de um projeto acadêmico e calcular sua própria média (\textit{Information Expert}). Ela não possui conhecimento sobre banco de dados, HTTP ou serialização JSON.
    \item \textbf{Nos Controladores}: Cada controlador em \texttt{src/server/controllers/} (\texttt{EquipeController}, \texttt{ProjetoController}, etc.) orquestra estritamente os fluxos dos casos de uso, sem executar comandos SQL ou manipular interfaces de usuário.
    \item \textbf{Na Persistência}: As classes de repositório em \texttt{src/server/repositories/} (\texttt{EquipeRepository}, \texttt{HackathonRepository}) dedicam-se exclusivamente ao mapeamento objeto-relacional e queries Knex/SQLite. Caso o banco seja migrado para PostgreSQL ou MongoDB, nenhuma linha de código de entidade ou rota precisa ser modificada.
    \item \textbf{Na Apresentação}: As rotas Fastify (\texttt{apiRoutes.ts}) cuidam exclusivamente do protocolo HTTP e códigos de status, enquanto as páginas React em \texttt{src/client/src/pages/} dedicam-se puramente à renderização reativa dos componentes de UI.
\end{itemize}

\subsection{Princípio Aberto/Fechado (Open/Closed Principle -- OCP)}
\textit{"Entidades de software devem ser abertas para extensão, mas fechadas para modificação"}.
\begin{itemize}
    \item \textbf{Hierarquia Extensível de Exceções de Domínio}: No arquivo \texttt{src/server/domain/errors.ts}, estruturamos a classe base abstrata \texttt{DomainError}. Novas regras de negócio e restrições (ex: \texttt{HackathonLotadoError}, \texttt{ParticipanteJaInscritoError}, \texttt{NotaInvalidaError}) são adicionadas estendendo essa classe base com seus respectivos códigos HTTP e mensagens semânticas.
    \item \textbf{Manipulador Global Fechado para Modificação}: Em \texttt{src/server/index.ts}, o manipulador de erros do Fastify (\texttt{fastify.setErrorHandler}) processa polimorficamente qualquer instância derivada de \texttt{DomainError} sem precisar ser editado a cada novo erro de domínio introduzido no sistema.
    \item \textbf{Composição de Esquemas Zod}: Novos esquemas de entrada estendem regras preexistentes (\texttt{.refine()}, \texttt{.safeParse()}) sem demandar refatoração nas entidades persistidas.
\end{itemize}

\subsection{Princípio da Substituição de Liskov (Liskov Substitution Principle -- LSP)}
\textit{"Subclasses devem ser substituíveis por suas superclasses sem afetar a correção do sistema"}.
\begin{itemize}
    \item \textbf{Intercambialidade de Erros de Domínio}: Qualquer especialização de erro de negócio (\texttt{HackathonLotadoError}, \texttt{RecursoNaoEncontradoError}) substitui perfeitamente \texttt{DomainError} em qualquer ponto da pilha de execução. Ambas satisfazem o contrato de propriedades \texttt{statusCode} e \texttt{message}, garantindo que consumidores de alto nível não necessitem de bifurcações condicionais com checagem de tipos concretos para manter a integridade da resposta HTTP.
\end{itemize}

\subsection{Princípio da Segregação de Interfaces (Interface Segregation Principle -- ISP)}
\textit{"Clientes não devem ser forçados a depender de interfaces que não utilizam"}.
\begin{itemize}
    \item \textbf{DTOs Granulares e Especializados}: Em vez de um objeto monolítico de transferência de dados, definimos contratos estritamente segregados em \texttt{schemas.ts}: \texttt{CriarHackathonInput}, \texttt{CadastrarParticipanteInput}, \texttt{InscreverEquipeInput}, \texttt{RegistrarProjetoInput} e \texttt{RegistrarAvaliacaoInput}.
    \item \textbf{Interfaces Especializadas por Papel de Ator}: O frontend consome apenas os contratos DTO estritamente necessários para o respectivo formulário do ator em foco (\texttt{/organizador}, \texttt{/estudante}, \texttt{/mentor}, \texttt{/jurado}), evitando o acoplamento a propriedades desnecessárias.
\end{itemize}

\subsection{Princípio da Inversão de Dependência (Dependency Inversion Principle -- DIP)}
\textit{"Módulos de alto nível não devem depender de módulos de baixo nível; ambos devem depender de abstrações"}.
\begin{itemize}
    \item \textbf{Desacoplamento entre Aplicação e Persistência}: Os controladores interagem com repositórios que expõem contratos de operações (\texttt{buscarPorId}, \texttt{criar}, \texttt{listar}), abstraindo os detalhes de baixo nível do dialeto SQL e das tabelas relacionais do SQLite. Isso permite substituir o mecanismo de persistência por adaptadores em memória ou mocks durante a execução de testes automatizados sem tocar na lógica de aplicação.
\end{itemize}

\section{Conclusão}

O projeto e a implementação desenvolvidos atendem aos requisitos propostos. A transição da especificação de requisitos para a modelagem orientada a objetos (Casos de Uso, Modelo de Domínio, DSS, Contratos e GRASP) resultou em uma arquitetura limpa, extensível e desacoplada, perfeitamente refletida no código-fonte em Node.js/SQLite e na interface web React.
