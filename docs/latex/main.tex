...
Para avaliar formalmente a coesão do software, adotamos a definição exata de \textbf{LCOM (\textit{Lack of Cohesion Between Methods})} apresentada nos slides da disciplina de Engenharia de Software da UFPR:
\begin{equation}
P = | \{ (f_1, f_2) \in M(C) \mid A(f_1) \text{ e } A(f_2) \text{ são conjuntos disjuntos} \} |
\end{equation}
\begin{equation}
\text{LCOM}(C) = P
\end{equation}
$\text{LCOM}(C)$ corresponde ao número de pares de métodos da classe $C$ que não usam atributos em comum, isto é, cuja interseção de atributos acessados é vazia ($A(f_1) \cap A(f_2) = \emptyset$). O patamar ideal de máxima coesão ocorre quando $\text{LCOM}(C) = 0$, atestando que não há métodos desconexos e que todos cooperam manipulando atributos compartilhados.

\subsection{Metodologia e Exemplo de Cálculo no Formato da Disciplina}

Para exemplificar a metodologia exata dos slides, a Tabela~\ref{tab:lcom_exemplo} detalha a análise de pares da classe de domínio \texttt{Projeto}:
...
\end{tabular}
\caption{Cálculo de LCOM para a Classe \texttt{Projeto} (onde $m_1 = \texttt{calcularNotaMedia}$, $m_2 = \texttt{adicionarAvaliacao}$, $m_3 = \texttt{obterTotalAvaliacoes}$, $m_4 = \texttt{obterResumo}$)}
\label{tab:lcom_exemplo}
\end{table}

Como nenhuma das interseções resultou em conjunto vazio ($\emptyset$), temos $P = 0$ e, portanto, $\text{LCOM}(\texttt{Projeto}) = 0$.

\subsection{Resultados Consolidados no Sistema}

A verificação foi automatizada no script \texttt{scripts/calculate\_lcom.ts} (executável por \texttt{npm run metrics:lcom} e validado na suíte \texttt{npm test}). A Tabela~\ref{tab:lcom} consolida os resultados para todas as 10 classes centrais:
...
\hline
\textbf{Classe} & \textbf{Camada Arquitetural} & $m$ & $a$ & \textbf{Total Pares} & $|P|$ (\textbf{LCOM}) & $|Q|$ & \textbf{Diagnóstico} \\ \hline
\texttt{Hackathon} & Entidade de Domínio & 4 & 6 & 6 & \textbf{0} & 6 & Alta Coesão (Ideal) \\ \hline
\texttt{Participante} & Entidade de Domínio & 3 & 5 & 3 & \textbf{0} & 3 & Alta Coesão (Ideal) \\ \hline
\texttt{Equipe} & Entidade de Domínio & 3 & 4 & 3 & \textbf{0} & 3 & Alta Coesão (Ideal) \\ \hline
\texttt{Projeto} & Entidade de Domínio & 4 & 3 & 6 & \textbf{0} & 6 & Alta Coesão (Ideal) \\ \hline
\texttt{Avaliacao} & Entidade de Domínio & 4 & 2 & 6 & \textbf{0} & 6 & Alta Coesão (Ideal) \\ \hline
\texttt{ItemClassificacao} & Entidade de Domínio & 3 & 5 & 3 & \textbf{0} & 3 & Alta Coesão (Ideal) \\ \hline
\texttt{EquipeController} & Controlador Aplicação & 3 & 3 & 3 & \textbf{0} & 3 & Alta Coesão (Ideal) \\ \hline
\texttt{ProjetoController} & Controlador Aplicação & 3 & 2 & 3 & \textbf{0} & 3 & Alta Coesão (Ideal) \\ \hline
\texttt{AvaliacaoController} & Controlador Aplicação & 2 & 3 & 1 & \textbf{0} & 1 & Alta Coesão (Ideal) \\ \hline
\texttt{ClassificacaoController} & Controlador Aplicação & 2 & 3 & 1 & \textbf{0} & 1 & Alta Coesão (Ideal) \\ \hline
\end{tabular}
\caption{Resultados Consolidados de LCOM no Sistema ($\text{LCOM} = P = 0$ em 100\% das classes)}
\label{tab:lcom}
\end{table}

\textbf{Diagnóstico Final}: Em 100\% das classes analisadas, obteve-se $\text{LCOM} = P = 0$. Nenhum par de métodos possui interseção vazia de atributos, comprovando que o projeto não possui classes infladas ou responsabilidades fragmentadas, aderindo rigorosamente ao princípio GRASP de Alta Coesão. Além disso, nas extensões teóricas mencionadas nos slides (LCOM2 e LCOM3), o sistema igualmente atinge valores ótimos.
